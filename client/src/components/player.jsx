...
const VideoPlayer = ({ message, downloadFile, onPlay }) => {
	...
	return (
		<div
			...
			className='relative max-w-3xl bg-cover object-cover bg-gray-900 rounded-lg overflow-hidden shadow-lg'
		>
			<div onClick={togglePlayPause} className='group/video'>
				<video
					ref={videoRef}
					src={currentSrc}
					className='w-[40rem] max-h-[28rem] bg-cover bg-center object-contain'
					controls={false}
				/>
			...
			<div className='absolute bottom-0 px-3 w-full bg-gray-800 bg-opacity-60 flex items-center justify-between p-2'>
				...
				{qualityMenuOpen && (
					<div className='absolute top-full right-0 bg-gray-800 text-white p-2 rounded shadow-lg'>
						<button
							onClick={() => changeQuality('low-quality.mp4')}
							className='block'
						>
							Low Quality
						</button>
						<button
							onClick={() => changeQuality('medium-quality.mp4')}
							className='block'
						>
							Medium Quality
						</button>
						<button
							onClick={() => changeQuality('high-quality.mp4')}
							className='block'
						>
							High Quality
						</button>
